...
public class MetadataProvider {
  ...
  public abstract static class MetadataRunnable implements Runnable {
    ...
    protected MetadataRunnable(final UserClientConnection connection, final DrillbitContext dContext,
  ...
  public static class CatalogsProvider extends MetadataRunnable {
    ...
    public CatalogsProvider(final UserClientConnection connection, final DrillbitContext dContext,
    ...
    public void run() {
      ...
      try (SchemaTreeProvider schemaTreeProvider = new SchemaTreeProvider(dContext)) {
        ...
        final PojoRecordReader<Catalog> records = (PojoRecordReader<Catalog>)
            getPojoRecordReader(CATALOGS, filter, schemaTreeProvider, connection.getSession());
        ...
        responseSender.send(new Response(RpcType.CATALOGS, respBuilder.build()));
      }
    }
  }
   ...
   */
  public static class SchemasProvider extends MetadataRunnable {
    private final GetSchemasReq req;

    public SchemasProvider(final UserClientConnection connection, final DrillbitContext dContext,
    ...
    public void run() {
      ...
      try (SchemaTreeProvider schemaTreeProvider = new SchemaTreeProvider(dContext)) {
        final PojoRecordReader<Schema> records = (PojoRecordReader<Schema>)
            getPojoRecordReader(SCHEMATA, filter, schemaTreeProvider, connection.getSession());
        ...
        responseSender.send(new Response(RpcType.SCHEMAS, respBuilder.build()));
      }
    }
  }
   ...
   */
  public static class TablesProvider extends MetadataRunnable {
    private final GetTablesReq req;

    public TablesProvider(final UserClientConnection connection, final DrillbitContext dContext,
    ...
    public void run() {
      ...
      try (SchemaTreeProvider schemaTreeProvider = new SchemaTreeProvider(dContext)) {
        final PojoRecordReader<Table> records =
            (PojoRecordReader<Table>)getPojoRecordReader(TABLES, filter, schemaTreeProvider, connection.getSession());
        ...
        responseSender.send(new Response(RpcType.TABLES, respBuilder.build()));
      }
    }
  }
   ...
   */
  public static class ColumnsProvider extends MetadataRunnable {
    private final GetColumnsReq req;

    public ColumnsProvider(final UserClientConnection connection, final DrillbitContext dContext,
    ...
    public void run() {
      ...
      try (SchemaTreeProvider schemaTreeProvider = new SchemaTreeProvider(dContext)) {
        final PojoRecordReader<Column> records =
            (PojoRecordReader<Column>)getPojoRecordReader(COLUMNS, filter, schemaTreeProvider, connection.getSession());
        ...
        responseSender.send(new Response(RpcType.COLUMNS, respBuilder.build()));
      }
    }
  }
  ...
  private static PojoRecordReader getPojoRecordReader(final InfoSchemaTableType tableType, final InfoSchemaFilter filter,
    ...
    final SchemaPlus rootSchema =
        provider.getRootSchema(userSession.getCredentials().getUserName(), newSchemaConfigInfoProvider(userSession));
