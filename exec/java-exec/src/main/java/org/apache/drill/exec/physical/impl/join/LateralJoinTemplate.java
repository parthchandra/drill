...
public abstract class LateralJoinTemplate implements LateralJoin {
  ...
  public int outputRecords(JoinRelType joinType) {
    ...
    while (leftRecordCount != 0) {
      ...
      if (outputIndex >= LateralJoinBatch.MAX_BATCH_SIZE) {
        break;
      }
  ...
  /**
   * This method is the core of the lateral join.For each left batch record looks for matching record
  ...
  private void resetAndGetNextLeft() {
    ...
    tracker.reset();
    RecordBatch.IterOutcome leftOutcome = outgoing.next(LateralJoinBatch.LEFT_INPUT, left);
    switch (leftOutcome) {
      case OK_NEW_SCHEMA:
        throw new DrillRuntimeException("Lateral join does not handle schema change. Schema change" +
            " found on the left side of Lateral Join.");
      case NONE:
      case NOT_YET:
      case STOP:
        leftRecordCount = 0;
