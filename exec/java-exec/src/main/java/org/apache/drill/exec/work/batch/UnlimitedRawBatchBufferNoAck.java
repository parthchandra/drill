...
/**
 * Class responsible for queueing RawFragmentBatches that
 * were received via RPC (currently via DrillClient channel, not data channel
 */
public class UnlimitedRawBatchBufferNoAck implements RawBatchBuffer {

  static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(UnlimitedRawBatchBufferNoAck.class);
  ...
  public void setFragmentCount(int fragmentCount) {
    if (fragmentCounterUsed.get()) {
      throw new RuntimeException("Attempted to change number of fragments after they are being consumed");
    }
    this.streamCounter.set(fragmentCount);
  }
  ...
  public RawFragmentBatch getNext(){
    ...
    if (b != null && b.getHeader().getIsLastBatch()) {
      // after we decremented at least one, we can not go back
      fragmentCounterUsed.set(true);
      streamCounter.getAndDecrement();
      if (streamCounter.get() == 0) {
        finished();
      }
    }
